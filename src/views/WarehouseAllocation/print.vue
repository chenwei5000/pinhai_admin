...
<script>
  ...
  export default {
    data() {
      return {
        primaryId: null,
        primary: {fromWarehouse: {name: ""}, toWarehouse: {name: ""}},
        items: [],
    ...
    methods: {
      initData() {
        if (this.primaryId) {
          ...
          url += "?relations=" + JSON.stringify(["product", "product.cartonSpec", "fromWarehouse", "toWarehouse"]);
