<template>
  ...
  <div class="ph-table">
    ...
    >
      ...
      <el-table-column prop="receivedCartonQty" label="收货箱数" width="80"></el-table-column>
      <el-table-column prop="receivedQty" label="收货件数" width="80"></el-table-column>
    ...
    methods: {
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (column.property == 'cartonQty' || column.property == 'receivedCartonQty') {
          ...
          if (column.property == 'qty' || column.property == 'receivedQty') {
            const values = data.map(item => Number(item[column.property]));
            if (!values.every(value => isNaN(value))) {
              sums[index] = values.reduce((prev, curr) => {
                const value = Number(curr);
