  ...
  <div class="ph-table">
    ...
    >
      <el-form-item label="仓库">
        <el-select
          filterable
          v-model="searchParam.warehouseId.value"
          size="mini"
          style="width: 200px"
          placeholder="请选择"
        >
...
export default {
  ...
  computed: {
    ...mapGetters(["device", "rolePower", "rolePower"]),
  ...
  data() {
    return {
      ...
      //抓数据 TODO: 根据实际情况调整
      url: "/materialRequisitions", // 资源URL
      countUrl: "/materialRequisitions/count", // 资源URL
  ...
  methods: {
    ...
    getList(shouldStoreQuery) {
      ...
      if (!countUrl) {
        console.warn(
          "countUrl 为空, 异常！如果不需要统计结果数量。请查找删除 countUrl 相关代码!"
        );
        return;
      }
